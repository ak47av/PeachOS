...
{
    while (*in != 0x00 && *in != 0x20) // 0x20 is a space
    {
        **out = *in;
        *out += 1;
        in += 1;
    ...
    if (*in == 0x20) // adds null terminator in space
    {
        **out = 0x00;
    }
}
...
void fat16_get_full_relative_filename(struct fat_directory_item *item, char *out, int max_len)
{
    ...
    char *out_tmp = out;
    fat16_to_proper_string(&out_tmp, (const char *)item->filename); // puts filename in out
    if (item->ext[0] != 0x00 && item->ext[0] != 0x20)
    {
        *out_tmp++ = '.';                                          // puts dot in out and increments pointer to out
        fat16_to_proper_string(&out_tmp, (const char *)item->ext); // puts extension in out
    }
}
...
// get fat table position
static uint32_t fat16_get_first_fat_sector(struct fat_private *private)
{
    return private->header.primary_header.reserved_sectors; // very first FAT table comes after the reserved sectors
}
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        // chec if sector is marked as bad
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total, void *out)
{
    ...
    int total_to_read = total > size_of_cluster_bytes ? size_of_cluster_bytes : total; // we can only read up to a cluster at a time
...
struct fat_item *fat16_new_fat_item_for_directory_item(struct disk *disk, struct fat_directory_item *item)
{
    ...
    if (item->attribute & FAT_FILE_SUBDIRECTORY)
    {
        ...
        f_item->directory = FAT_ITEM_TYPE_DIRECTORY;
    }
...
{
    int res = 0;
    struct fat_file_descriptor* desc = private;    // private is not fat_private, it is the descriptor
    struct fat_item* desc_item = desc->item;
    if(desc_item->type!=FAT_ITEM_TYPE_FILE)
    {
        res = -EINVARG;
